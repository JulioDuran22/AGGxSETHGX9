...
try {
  ...
  const info = `\n╭─────────────────────────────◉\n│ ${chalk.red.bgBlueBright.bold(
    ...
    `Autor: ${
      packageJsonObj.author && packageJsonObj.author.name
        ? packageJsonObj.author.name
        : packageJsonObj.author || "N/A"
    } (@leo xzzsy)`
  )}\n│「 🐢 」${chalk.cyan(
    "Colaborador: Bryan ofc Davidryze y mas"
...
async function startBot() {
  ...
  sock.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      if (statusCode !== DisconnectReason.loggedOut) {
        ...
        console.log(
          "[Sesión cerrada] Borra la carpeta de credenciales y vuelve a vincular."
        );
      }
  ...
  sock.ev.on("group-participants.update", async (ev) => {
    try {
      ...
      const type =
        action === "add" ? "welcome" : action === "remove" ? "bye" : null;
      if (!type) return;
      ...
      for (const p of participants) {
        try {
            ...
            try {
              const cfgDefaults =
                global.chatDefaults && typeof global.chatDefaults === "object"
                  ...
                  : {};
              global.db = global.db || {
                data: { users: {}, chats: {}, settings: {}, stats: {} },
              };
              ...
              if (
                !("bienvenida" in global.db.data.chats[id]) &&
                "welcome" in cfgDefaults
              )
          ...
          const code =
            e?.data || e?.output?.statusCode || e?.output?.payload?.statusCode;
          if (code === 403) {
            continue;
          }
