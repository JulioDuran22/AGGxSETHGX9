async function handler(m, { conn, args, usedPrefix, command }) {
  const ctxErr = global.rcanalx || { contextInfo: { externalAdReply: { title: '❌ Error', body: 'AGG x ꜱᴇᴛʜɢx9 ', thumbnailUrl: 'https://qu.ax/QGAVS.jpg', sourceUrl: global.canalOficial || '' }}}
  const ctxWarn = global.rcanalw || { contextInfo: { externalAdReply: { title: '⚠️ Advertencia', body: 'AGG x ꜱᴇᴛʜɢx9', thumbnailUrl: 'https://qu.ax/QGAVS.jpg', sourceUrl: global.canalOficial || '' }}}
  const ctxOk = global.rcanalr || { contextInfo: { externalAdReply: { title: '✅ Transferencia', body: 'AGG x ꜱᴇᴛʜɢx9', thumbnailUrl: 'https://qu.ax/QGAVS.jpg', sourceUrl: global.canalOficial || '' }}}
  
  const currency = global.currency || 'Yenes'

  if (!db.data.chats[m.chat].economy && m.isGroup) {
    return conn.reply(m.chat, `🍙📚 *AGG - Sistema de Economía*\n\n❌ Los comandos de economía están desactivados en este grupo\n\n*Administrador*, activa la economía con:\n${usedPrefix}economy on\n\n📖 "No puedo procesar transferencias si la economía está desactivada..."`, m, ctxErr)
  }
  ...
  await conn.reply(m.chat, 
    ...
    `🍱✨ "¡Gracias por usar el sistema bancario de AGG!"`, 
    m, 
    { ...ctxOk, mentions: [who] }
  )
