...
function isSubBotConnected(jid) {
  return global.conns.some(
    (sock) =>
      sock?.user?.jid && sock.user.jid.split("@")[0] === jid.split("@")[0]
  );
}
let handler = async (m, { conn, args, usedPrefix, command, isOwner }) => {
  ...
  if (socklimit >= 50) {
    return m.reply(
      `ꕥ No se han encontrado espacios para *Sub-Bots* disponibles.`
    );
  }
...
export async function yukiJadiBot(options) {
  ...
  try {
    ...
    conn.reply(
      m.chat,
      `ꕥ Use correctamente el comando » ${usedPrefix + command}`,
      m
    );
    return;
  }
  ...
  exec(comb.toString("utf-8"), async (err, stdout, stderr) => {
    ...
    setTimeout(async () => {
      if (!sock.user) {
        ...
        console.log(
          `[AUTO-LIMPIEZA] Sesión ${path.basename(
            pathYukiJadiBot
          )} eliminada credenciales invalidos.`
        );
      }
    }, 60000);
      ...
      if (connection === "close") {
          ...
          console.log(
            chalk.bold.magentaBright(
              `\n╭┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄⟡\n┆ La conexión (+${path.basename(
                pathYukiJadiBot
              )}) fue cerrada inesperadamente. Intentando reconectar...\n╰┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄⟡`
            )
          );
        ...
        if (reason === 408) {
          console.log(
            chalk.bold.magentaBright(
              `\n╭┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄⟡\n┆ La conexión (+${path.basename(
                pathYukiJadiBot
              )}) se perdió o expiró. Razón: ${reason}. Intentando reconectar...\n╰┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄⟡`
            )
          );
          await creloadHandler(true).catch(console.error);
        }
        if (reason === 440) {
          ...
          try {
            ...
            console.error(
              chalk.bold.yellow(
                `⚠︎ Error 440 no se pudo enviar mensaje a: +${path.basename(
                  pathYukiJadiBot
                )}`
              )
            );
          ...
          console.log(
            chalk.bold.magentaBright(
              `\n╭┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄⟡\n┆ La sesión (+${path.basename(
                pathYukiJadiBot
              )}) fue cerrada. Credenciales no válidas o dispositivo desconectado manualmente.\n╰┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄⟡`
            )
          );
          try {
            if (options.fromCommand)
            ...
            console.error(
              chalk.bold.yellow(
                `⚠︎ Error 405 no se pudo enviar mensaje a: +${path.basename(
                  pathYukiJadiBot
                )}`
              )
            );
          ...
          ? await conn.sendMessage(
              m.chat,
              {
                text: isSubBotConnected(m.sender)
                  ...
                  : `❀ Has registrado un nuevo *Sub-Bot!* [@${
                      m.sender.split("@")[0]
